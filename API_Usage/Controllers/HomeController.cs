...
namespace API_Usage.Controllers
{
    public class HomeController : Controller
    {
        ...
        ****/
        ...
        {
            List<Top_Gainers> gainerList = getGainers();
        ...
        {
            List<Top_Gainers> gainer = JsonConvert.DeserializeObject<List<Top_Gainers>>(TempData["Gainer"].ToString());
            foreach (Top_Gainers item in gainer)
            {

                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add company only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.Top_Gainers.Where(c => c.ID.Equals(item.ID)).Count() == 0)
                {
                    dbContext.Top_Gainers.Add(item);
                }
            }
            dbContext.SaveChanges();
        ...
        public List<Top_Gainers> getGainers()
        {
            string IEXTrading_API_PATH = BASE_URL + "stock/market/list/gainers";
            string Gainers_list = "";
            List<Top_Gainers> gainersList = null;
            ...
            if (response.IsSuccessStatusCode)
            {
                Gainers_list = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            }
            if (!Gainers_list.Equals(""))
            {
                List<Top_Gainers> gL = JsonConvert.DeserializeObject<List<Top_Gainers>>(Gainers_list, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
                gainersList = gL;
            }
            return gainersList;
        }
        ...
        public IActionResult sectorL()
        {
            List<Sector> sector = new List<Sector>();
            sector = getSector();
            TempData["SectorL"] = JsonConvert.SerializeObject(sector);
        ...
        {
            List<Sector> sector = JsonConvert.DeserializeObject<List<Sector>>(TempData["SectorL"].ToString());
        ...
        public List<Top_Gainers> getSectorStock(string sector)
        {
            ...
            if (!gList.Equals(""))
            {
                sectorStock = JsonConvert.DeserializeObject<List<Top_Gainers>>(gList, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
        ...
        public FinancialList getFinancials(string symbols)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                Financial_List = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            }
            if (!Financial_List.Equals(""))
            {
                finance = JsonConvert.DeserializeObject<FinancialList>(Financial_List, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
            }
            return finance;
        }
        ...
        public IActionResult News()
        {
            ...
            return View("News_List", newsList);
        }
        public List<News> getNews()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                News_list1 = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            }
            if (!News_list1.Equals(""))
            {
                List<News> gL = JsonConvert.DeserializeObject<List<News>>(News_list1, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
                newsList = gL;
            }
            return newsList;
        }
        ...
        {
            List<News> news = JsonConvert.DeserializeObject<List<News>>(TempData["News"].ToString());
            foreach (News item in news)
            {

                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add company only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.newsList.Where(c => c.ID.Equals(item.ID)).Count() == 0)
                {
                    dbContext.newsList.Add(item);
                }
            }
            dbContext.SaveChanges();
            ViewBag.dbSuccessComp = 1;
            return View("News_List", news);
        }
    }
}
